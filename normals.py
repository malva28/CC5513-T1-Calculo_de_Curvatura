...
def calculate_vertex_face_angles(mesh: openmesh.TriMesh) -> np.ndarray:
...
def calculate_vertex_normals_by_angle(mesh: openmesh.TriMesh) -> np.ndarray:
    face_normals = calculate_face_normals(mesh)
    vertex_face_angles = calculate_vertex_face_angles(mesh)
...
ps.get_surface_mesh("my mesh").add_vector_quantity("vertex_normals", calculate_vertex_normals_by_angle(mesh))
ps.show()
